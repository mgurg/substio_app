...
class FacebookPageParser:
    """Parser for Facebook page HTML files."""
    ...
    def normalize_duration(self, duration_str: str) -> str:
        """Normalize duration string by reversing if it starts with a letter."""
    ...
    def calculate_post_timestamp(self, duration_str: str, base_timestamp: str) -> str:
        ...
        return new_dt.strftime("%Y-%m-%dT%H_%M_%S.%f")[:-3] + 'Z'

    def extract_links_and_author(self, div, base_timestamp: str) -> Tuple[str, str, str, str, str]:
        """Extract author, author link, post link, and timestamp from message div."""
        author = ""
        author_link = ""
        post_link = ""
        timestamp = ""
    ...
    def save_to_json(self, posts: List[FacebookPost], base_filename: str) -> str:
        """Save posts to JSON file in the output directory."""
        # Extract timestamp from base filename for the output filename
    ...
    def print_post(self, post: FacebookPost) -> None:
        ...
        print(f"Message {post.index} (aria-posinset={post.posinset}):")
        if post.author:
            print(f"Author: {post.author}")
        if post.author_link:
            print(f"  → Author Link: {post.author_link}")
        if post.post_link:
            print(f"  → Post Link: {post.post_link}")
        if post.timestamp:
            print(f"Timestamp: {post.timestamp}")
        if post.delay:
            print(f"Delay: {post.delay}")
        print(post.content)
        print("-" * 40)
